...
//		int badHash = (int) Double.parseDouble(key.toString().split(" ")[0]);
//		return badHash; 
	}
...
//		int start = (i+10) % M;  	// linear probing starting point + 10
		int start = (i+(M/2)) % M;  // linear probing starting point +1/2 of table size
		putCount++;
	...
	public static void putExperiment( double loadFactor, String file, boolean checkIntegrity) { 
		...
		for ( int expSize = 1000; expSize <= 10000; expSize+=1000) {
			...
			// COMMENT THE BELOW INTEGRITY TEST OUT FOR EXPERIMENTS 2 & 3 (Change to fale in main function)
			if (checkIntegrity)
				if ( aHashST.check()) 
	...
	public static void main(String[] args) {
		putExperiment( 0.8, "data/hw6.txt", false);   // true -  check integrity 
		StdOut.println("-----------------");
