 ...
 *
 *************************************************************************/
public class BST2ClassNotes<K extends Comparable<? super K>, V> {
	...
	// Lecture Notes *BEN CODED THE FOLLOWING GET FUNCTION
...
//		return null;
	...
	private V get(K key, Node<K,V> x) {
		
		if ( x == null)
			return null;
		int res = x.key.compareTo(key);
		if ( res == 0)
			return x.val;
		if ( res < 0) {
